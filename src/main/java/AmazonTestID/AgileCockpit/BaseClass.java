...
public class BaseClass {
	public void getpart(int product ) {
		...
		List<WebElement> itemList = AppleWatch.driver.findElements(By.xpath("//a/h2[contains(@class,'s-access-title')]"));
		
for (int i = 0; i < itemList.size(); i++) {
			
			if(i==product) {
				System.out.println(itemList.get(i).getText());
				List<WebElement> price = AppleWatch.driver.findElements(By.xpath("//div[@class='a-fixed-left-grid-col a-col-right']//div//div//span[@class='a-size-base a-color-price s-price a-text-bold']"));
	...
	public void rating(int product) {
		 List<WebElement> rating = AppleWatch.driver.findElements(By.xpath("((//div[@class='a-fixed-left-grid-col a-col-right'])[3]//a[@class='a-size-small a-link-normal a-text-normal'])[1]"));
		
		for (int j = 0; j < rating.size(); j++) {
				if(j==product) {
					System.out.println(rating.get(j).getText());
					break;
				}
		}
			}
